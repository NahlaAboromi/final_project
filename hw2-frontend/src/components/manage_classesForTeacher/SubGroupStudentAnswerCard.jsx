...
const SubGroupStudentAnswerCard = ({ answer, student = {}, isDark }) => {
  const { studentId, id, analysisResult } = answer || {};
  const realStudentId = studentId || id;
  ...
  useEffect(() => {
    const fetchFullStudentData = async () => {
      ...
      if (realStudentId && (missingUsername || missingPic)) {
        try {
          const res = await fetch(`/api/students/${realStudentId}`);
  ...
  }, [realStudentId, student]);

  if (!ready || !analysisResult) return null;

  const profilePic = fullStudent.profilePic || student.profilePic;
  const username =
    fullStudent.username ||
    fullStudent.fullName ||
    student.username ||
    student.fullName ||
    t('unknown', 'Unknown Student');

  const validAttempts = [answer];
  ...
  const averageScore = Number(analysisResult?.overallScore || 0).toFixed(1);
  ...
  const pct = (score) =>
    Math.max(0, Math.min(100, (Number(score || 0) / 5) * 100));
      ...
      <div className={`p-4 ${isDark ? 'bg-slate-700 text-slate-200' : 'bg-white text-gray-800'}`}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {Object.entries(ar)
              ...
              .map(([key, val]) => (
                <div
                  key={key}
                  className={`p-3 rounded-md ${isDark ? 'bg-slate-800' : 'bg-slate-50'} border-l-4 ${getScoreBadgeColor(val.score)}`}
                >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
          <div className={`${isDark ? 'bg-slate-800' : 'bg-green-50'} p-3 rounded-md`}>
            <h4 className={`${isDark ? 'text-green-300' : 'text-green-800'} font-bold mb-2`}>
              💪 {t('strengths')}
            </h4>
          ...
          <div className={`${isDark ? 'bg-slate-800' : 'bg-yellow-50'} p-3 rounded-md`}>
            <h4 className={`${isDark ? 'text-yellow-300' : 'text-yellow-800'} font-bold mb-2`}>
        ...
        <div className={`${isDark ? 'bg-slate-800' : 'bg-blue-50'} p-3 rounded-md`}>
          <h4 className={`${isDark ? 'text-blue-300' : 'text-blue-800'} font-bold mb-2`}>
            💡 {t('suggestedIntervention')}
          </h4>
  ...
  return (
    ...
    >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
            {categoryKeys.map((key) => {
              const score = getScore(answer, key);
              const color = getScoreHex(score);
              ...
              return (
                <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 220 }}>
                  ...
                  <span style={{ fontSize: 11, color: textSub, width: 90 }}>
                    {displayNames[key]}
                  </span>

                  <div style={{ flex: 1, height: 5, background: isDark ? '#334155' : '#e2e8f0', borderRadius: 99 }}>
                    <div style={{ height: '100%', width: `${percent}%`, background: color, borderRadius: 99 }} />
                  </div>

                  <span style={{ fontWeight: 800, fontSize: 13, color, minWidth: 32 }}>
                    {score}
      ...
      <div style={{ padding: '16px 24px' }}>
        <div style={{ fontWeight: 700, fontSize: 14, color: textSub, marginBottom: 12 }}>
          {lang === 'he' ? 'ניסיון אחד' : 'One Attempt'}
        </div>

        {validAttempts.map((attempt, index) => {
          const attemptId = attempt?._id || `${realStudentId}-${attempt?.submittedAt}-${index}`;
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 14, flex: 1 }}>
                  ...
                  <div>
                    <div style={{ fontWeight: 700, fontSize: 14, color: textMain }}>
                      {lang === 'he' ? 'תשובת הסטודנט' : 'Student Answer'}
                    </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ textAlign: 'center', background: `${color}18`, borderRadius: 10, padding: '6px 14px' }}>
                    ...
                    <div style={{ fontWeight: 900, fontSize: 20, color }}>
                      {overall}
                    </div>
                  </div>
...
export default SubGroupStudentAnswerCard;